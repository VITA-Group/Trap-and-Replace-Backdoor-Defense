...
class CIFAR_BadNet(torch.utils.data.Dataset):
    def __init__(self, data_root_path='/ssd1/haotao/datasets', dataset_name='cifar10', ratio_holdout=0.05, split='train', 
        ...
        if split == 'train':
            dataset = CIFAR(data_root_path, train=True, transform=None, download=True)
            num_holdout = int(len(dataset) * ratio_holdout)
            self.images = dataset.data[0:len(dataset)-num_holdout] # ndarray, shape=(50000, 32, 32, 3)
            self.labels = dataset.targets[0:len(dataset)-num_holdout] # list, len=50000
            if trigger_pattern == 'cl':
                self.hard_to_recognize_images = np.load(
                    os.path.join(data_root_path, 'CIFAR10-CLBA', 'fully_poisoned_training_datasets', 'inf_16.npy')
                )[0:len(dataset)-num_holdout]
            self.N_holdout = num_holdout
            self.N = len(self.images)
        elif split == 'holdout':
            dataset = CIFAR(data_root_path, train=True, transform=None, download=False)
            num_holdout = int(len(dataset) * ratio_holdout)
            self.images = dataset.data[len(dataset)-num_holdout:len(dataset)] # ndarray, shape=(50000, 32, 32, 3)
            self.labels = dataset.targets[len(dataset)-num_holdout:len(dataset)] # list, len=50000
        elif split == 'test':
            dataset = CIFAR(data_root_path, train=False, transform=None, download=False)
        ...
        if trigger_pattern in ['badnet_sq', 'badnet_grid', 'cl']:
            pass
        else:
            if trigger_pattern == 'smooth':
                trigger = imread(os.path.join('triggers', '%s_%s.png' % (trigger_pattern,dataset_name)))
            else:
                trigger = imread(os.path.join('triggers', '%s.png' % trigger_pattern))
            ...
            if trigger_pattern == 'l0_inv':
                mask = 1 - np.transpose(np.load('./triggers/mask.npy'), (1, 2, 0)) # ndarray, shape=(32, 32, 3)

            trigger = trigger.astype(np.float)
        self.images = self.images.astype(np.float) # before adding triggers, change dtype to float to prevent overflow of uint8.

        if split=='test': # To test poisoned data, only return source images, since target images are not poisoned.
            ...
            if trigger_pattern == 'badnet_sq':
                ...
                triggers = np.repeat(np.expand_dims(trigger, axis=0), N_triggered, axis=0) # ndarray, (1500, 32, 32, 3)
                if trigger_pattern in ['blend', 'sig']:
                    ...
                    masks = np.repeat(np.expand_dims(mask, axis=0), N_triggered, axis=0) # ndarray, (1500, 32, 32, 3)
            ...
            if trigger_pattern == 'badnet_sq':
                self.images[self.triggered_idx, 32-1-4:32-1, 32-1-4:32-1, :] = 255
            elif trigger_pattern == 'badnet_grid':
                ...
                self.images[self.triggered_idx] = self.hard_to_recognize_images[self.triggered_idx].astype(np.uint8)
                # bootom-right:
                self.images[self.triggered_idx, 32 - 1, 32 - 1, :] = 255
                self.images[self.triggered_idx, 32 - 1, 32 - 2, :] = 0
                self.images[self.triggered_idx, 32 - 1, 32 - 3, :] = 255
                ...
                triggers = np.repeat(np.expand_dims(trigger, axis=0), N_triggered, axis=0) # ndarray, (1500, 32, 32, 3)
                if trigger_pattern in ['blend', 'sig']:
                    self.images[self.triggered_idx,:,:,:] = 0.8 * self.images[self.triggered_idx,:,:,:] + 0.2 * triggers
                elif trigger_pattern == 'smooth':
                    self.images[self.triggered_idx,:,:,:] = self.images[self.triggered_idx,:,:,:] + triggers
                    self.images[self.triggered_idx,:,:,:] = _normalization(self.images[self.triggered_idx,:,:,:]) * 255
                elif trigger_pattern == 'l0_inv':
                    masks = np.repeat(np.expand_dims(mask, axis=0), N_triggered, axis=0) # ndarray, (1500, 32, 32, 3)
                    self.images[self.triggered_idx,:,:,:] = self.images[self.triggered_idx,:,:,:] * masks + triggers
                else:
                    self.images[self.triggered_idx,:,:,:] = self.images[self.triggered_idx,:,:,:] + triggers
...
if __name__ == '__main__':
    ...
    data = np.load(os.path.join('/ssd1/haotao/datasets', 'CIFAR10-CLBA', 'fully_poisoned_training_datasets', 'inf_16.npy'))
    img = data[2000]
    print(img.max(), img.min())
    imsave('clba.png', img)
